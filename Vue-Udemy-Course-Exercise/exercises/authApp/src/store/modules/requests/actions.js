export default {
  async contactCoach(context, payload) {
    ...
    const response = await fetch(
      ...
      `https://find-a-coach-2a164-default-rtdb.firebaseio.com/requests/${payload.coachId}.json`,
      {
        method: 'POST',
  ...
  async fetchRequests(context) {
    ...
    const response = await fetch(
      ...
      //   token
      `https://find-a-coach-2a164-default-rtdb.firebaseio.com/requests/${coachId}.json?auth=`+ token
    );
    const responseData = await response.json();
